...
@section('content')
    <section id="main-content">
        <section class="wrapper">
            ...
            <div class="row mt">
                <div class="col-lg-12">
                    <h4><i class="fa fa-angle-right"></i> <a href="{{route('admin.product.index')}}" type="button" class="btn btn-theme">Back All Product</a></h4>
                    <div class="form-panel">
                        <div class=" form">
                            <form class="cmxform form-horizontal style-form" id="commentForm" action="{{route('admin.product.update',$product->id)}}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group ">
                                    <label for="cname" class="control-label col-lg-2"> Product Category</label>
                                    <div class="col-lg-8">
                                        <select class="form-control" name="category_id">
                                            @foreach(App\Category::orderBy('name','asc')->where('parent_id', NULL)->get() as $parent)
                                                <option value="{{$parent->id}}" {{$parent->id==$product->category_id ? 'selected' : ''}}>{{$parent->name}}</option>
                                                @foreach(App\Category::orderBy('name','asc')->where('parent_id', $parent->id)->get() as $child)
                                                    <option value="{{$child->id}}" {{$child->id==$product->category_id ? 'selected' : ''}}><i class="fa fa-long-arrow-right"></i>---{{$child->name}}</option>
                                                @endforeach
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="form-group ">
                                    <label for="cname" class="control-label col-lg-2">Product Quantity (required)</label>
                                    <div class="col-lg-8">
                                        <input class="form-control" id="name" name="quantity" type="text" value="{{$product->quantity}}"/>
                                    </div>
                                </div>
                                <div class="form-group ">
                                    <label for="cname" class="control-label col-lg-2">Product Price (required)</label>
                                    <div class="col-lg-8">
                                        <input class="form-control" id="name" name="price" type="text" value="{{$product->price}}"/>
                                    </div>
                                </div>
                                <div class="form-group ">
                                    <label for="ccomment" class="control-label col-lg-2">Product Description (required)</label>
                                    <div class="col-lg-8">
                                        <textarea class="form-control" id="comment" name="description" rows="10">{{$product->description}}</textarea>
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label class="control-label col-md-2">Product Image Upload</label>
                                    <div class="col-md-10">
                                        <div class="row">
                                            <div class="col-md-2">
